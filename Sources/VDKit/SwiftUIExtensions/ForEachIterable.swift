...
extension ForEach: IterableViewType where Content: View {
	public var count: Int { data.reduce(0) { $0 + content($1).contentCount } }
	...
	public func iterate<V: IterableViewVisitor>(with visitor: V) {
		data.map(content).forEach { $0.iterateContent(with: visitor) }
	}
}
