...
#import "include/_VDDelegateProxy.h"
#import "include/_VD.h"
#import "include/_VDObjCRuntime.h"

@interface _VDDelegateProxy () {
...
@implementation _VDDelegateProxy
...
+(NSSet*)collectVoidSelectorsForProtocol:(Protocol *)protocol {
    ...
    for (unsigned int i = 0; i < protocolMethodCount; ++i) {
        ...
        if (VD_is_method_with_description_void(method)) {
...
+(void)initialize {
    @synchronized (_VDDelegateProxy.class) {
...
-(void)forwardInvocation:(NSInvocation *)anInvocation {
    BOOL isVoid = VD_is_method_signature_void(anInvocation.methodSignature);
    NSArray *arguments = nil;
    if (isVoid) {
        arguments = VD_extract_arguments(anInvocation);
