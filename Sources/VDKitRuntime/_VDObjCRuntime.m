...
#import "include/_VD.h"
#import "include/_VDObjCRuntime.h"
...
BOOL VDAbortOnThreadingHazard = NO;
...
typedef BOOL (^VDInterceptWithOptimizedObserver)(VDObjCRuntime * __nonnull self, Class __nonnull class, SEL __nonnull selector, NSErrorParam error);
...
#define VD_PREFIX @"_VD_namespace_"

#define VD_ARG_id(value)           ((value) ?: [NSNull null])
#define VD_ARG_char(value)         [NSNumber numberWithChar:value]
#define VD_ARG_short(value)        [NSNumber numberWithShort:value]
#define VD_ARG_int(value)          [NSNumber numberWithInt:value]
#define VD_ARG_long(value)         [NSNumber numberWithLong:value]
#define VD_ARG_BOOL(value)         [NSNumber numberWithBool:value]
#define VD_ARG_SEL(value)          [NSNumber valueWithPointer:value]
#define VD_ARG_rx_uchar(value)     [NSNumber numberWithUnsignedInt:value]
#define VD_ARG_rx_ushort(value)    [NSNumber numberWithUnsignedInt:value]
#define VD_ARG_rx_uint(value)      [NSNumber numberWithUnsignedInt:value]
#define VD_ARG_rx_ulong(value)     [NSNumber numberWithUnsignedLong:value]
#define VD_ARG_rx_block(value)     ((id)(value) ?: [NSNull null])
#define VD_ARG_float(value)        [NSNumber numberWithFloat:value]
#define VD_ARG_double(value)       [NSNumber numberWithDouble:value]
...
NSString * __nonnull const VDObjCRuntimeErrorDomain   = @"VDObjCRuntimeErrorDomain";
NSString * __nonnull const VDObjCRuntimeErrorIsKVOKey = @"VDObjCRuntimeErrorIsKVOKey";

BOOL VD_return_type_is_supported(const char *type) {
    if (type == nil) {
        return NO;
    }
...
static BOOL VD_method_has_supported_return_type(Method method) {
    ...
    return VD_return_type_is_supported(methodSignature.methodReturnType);
}

SEL __nonnull VD_selector(SEL __nonnull selector) {
    NSString *selectorString = NSStringFromSelector(selector);
    return NSSelectorFromString([VD_PREFIX stringByAppendingString:selectorString]);
}
...
id __nonnull VD_extract_argument_at_index(NSInvocation * __nonnull invocation, NSUInteger index) {
...
NSArray *VD_extract_arguments(NSInvocation *invocation) {
    ...
    for (NSUInteger index = HIDDEN_ARGUMENT_COUNT; index < numberOfArguments; ++index) {
        [arguments addObject:VD_extract_argument_at_index(invocation, index) ?: [NSNull null]];
    }
...
static BOOL VD_forward_invocation(id __nonnull __unsafe_unretained self, NSInvocation *invocation) {
    SEL originalSelector = VD_selector(invocation.selector);

    id<VDMessageSentObserver> messageSentObserver = objc_getAssociatedObject(self, originalSelector);

    if (messageSentObserver != nil) {
        NSArray *arguments = VD_extract_arguments(invocation);
...
static BOOL VD_responds_to_selector(id __nonnull __unsafe_unretained self, SEL selector) {
...
static NSMethodSignatureRef VD_method_signature(id __nonnull __unsafe_unretained self, SEL selector) {
    Class class = object_getClass(self);
...
static NSString * __nonnull VD_method_encoding(Method __nonnull method) {
...
@interface VDObjCRuntime: NSObject
...
+(VDObjCRuntime*)instance;

-(void)performLocked:(void (^)(VDObjCRuntime* __nonnull))action;
...
+(void)registerOptimizedObserver:(VDInterceptWithOptimizedObserver)registration encodedAs:(SEL)selector;

@end

/**
 All API methods perform work on locked instance of `VDObjCRuntime`. In that way it's easy to prove
 that every action is properly locked.
 */
IMP __nullable VD_ensure_observing(id __nonnull target, SEL __nonnull selector, NSErrorParam error) {
    ...
    @synchronized(target) {
        ...
        @synchronized([target class]) {
            [[VDObjCRuntime instance] performLocked:^(VDObjCRuntime * __nonnull self) {
...
#define FORWARD_BODY(invocation)                        if (VD_forward_invocation(self, NAME_CAT(_, 0, invocation))) { return; }

#define RESPONDS_TO_SELECTOR_BODY(selector)             if (VD_responds_to_selector(self, NAME_CAT(_, 0, selector))) return YES;

#define CLASS_BODY(...)                                 return actAsClass;

#define METHOD_SIGNATURE_FOR_SELECTOR_BODY(selector)                                            \
    NSMethodSignatureRef methodSignature = VD_method_signature(self, NAME_CAT(_, 0, selector)); \
    if (methodSignature != nil) {                                                               \
        return methodSignature;                                                                 \
    }

#define DEALLOCATING_BODY(...)                                                        \
    id<VDDeallocatingObserver> observer = objc_getAssociatedObject(self, rxSelector); \
    if (observer != nil && observer.targetImplementation == thisIMP) {                \
        [observer deallocating];                                                      \
    }

#define OBSERVE_BODY(...)                                                              \
    id<VDMessageSentObserver> observer = objc_getAssociatedObject(self, rxSelector);   \
                                                                                       \
    if (observer != nil && observer.targetImplementation == thisIMP) {                 \
...
#define BUILD_ARG_WRAPPER(type)                   VD_ARG_ ## type                                                     //VD_ARG_ ## type

#define CAT(_1, _2, head, tail)                   VD_CAT2(head, tail)
#define SEPARATE_BY_COMMA(_1, _2, head, tail)     head, tail
#define SEPARATE_BY_SPACE(_1, _2, head, tail)     head tail
#define SEPARATE_BY_UNDERSCORE(head, tail)        VD_CAT2(VD_CAT2(head, _), tail)

#define UNDERSCORE_TYPE_CAT(_1, index, type)      VD_CAT2(_, type)                                                    // generates -> _type
...
#define EXAMPLE_PARAMETER(_1, index, type)        VD_CAT2(_, type):(type)SEPARATE_BY_UNDERSCORE(type, index)          // generates -> _type:(type)type_0
#define SELECTOR_PART(_1, index, type)            VD_CAT2(_, type:)                                                   // generates -> _type:

#define COMMA_DELIMITED_ARGUMENTS(...)            VD_FOREACH(_, SEPARATE_BY_COMMA, NOT_NULL_ARGUMENT_CAT, ## __VA_ARGS__)
#define ARGUMENTS(...)                            VD_FOREACH_COMMA(_, NAME_CAT, ## __VA_ARGS__)
#define DECLARE_ARGUMENTS(...)                    VD_FOREACH_COMMA(_, TYPE_AND_NAME_CAT, ## __VA_ARGS__)

// optimized observe methods

#define GENERATE_SELECTOR_IDENTIFIER(...)         VD_CAT2(exampleSelector, VD_FOREACH(_, CAT, UNDERSCORE_TYPE_CAT, ## __VA_ARGS__))

#define GENERATE_METHOD_IDENTIFIER(...)           VD_CAT2(swizzle, VD_FOREACH(_, CAT, UNDERSCORE_TYPE_CAT, ## __VA_ARGS__))
...
#define BUILD_EXAMPLE_METHOD(return_value, ...) \
    +(return_value)VD_CAT2(VD_CAT2(example_, return_value), VD_FOREACH(_, SEPARATE_BY_SPACE, EXAMPLE_PARAMETER, ## __VA_ARGS__)) {}

#define BUILD_EXAMPLE_METHOD_SELECTOR(return_value, ...) \
    VD_CAT2(VD_CAT2(example_, return_value), VD_FOREACH(_, SEPARATE_BY_SPACE, SELECTOR_PART, ## __VA_ARGS__))
...
#define SWIZZLE_OBSERVE_METHOD_BODY(return_value, ...)                                                                                                       \
       ...
       [self registerOptimizedObserver:^BOOL(VDObjCRuntime * __nonnull self, Class __nonnull class,                                                     \
...
method_prototype                                                                                                         \
    __unused SEL rxSelector = VD_selector(selector);                                                                     \
...
@interface VDObjCRuntime (InfrastructureMethods)
@end

// MARK: Infrastructure Methods

@implementation VDObjCRuntime (InfrastructureMethods)
...
@interface VDObjCRuntime (swizzle)

@end

@implementation VDObjCRuntime(swizzle)
...
@implementation VDObjCRuntime

static VDObjCRuntime *_instance = nil;
static NSMutableDictionary<NSString *, VDInterceptWithOptimizedObserver> *optimizedObserversByMethodEncoding = nil;
...
+(void)initialize {
    _instance = [[VDObjCRuntime alloc] init];
    defaultTypeID = CFGetTypeID((CFTypeRef)VDObjCRuntime.class); // just need a reference of some object not from CF
...
+(void)registerOptimizedObserver:(VDInterceptWithOptimizedObserver)registration encodedAs:(SEL)selector {
    ...
    NSString *methodEncoding = VD_method_encoding(exampleEncodingMethod);
...
-(IMP __nullable)ensurePrepared:(id __nonnull)target forObserving:(SEL __nonnull)selector error:(NSErrorParam)error {
    ...
    if (instanceMethod == nil) {
        VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                           code:VDObjCRuntimeErrorSelectorNotImplemented
                                       userInfo:nil], nil);
    }
    ...
    ||  selector == @selector(respondsToSelector:)) {
        VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                           code:VDObjCRuntimeErrorObservingPerformanceSensitiveMessages
                                       userInfo:nil], nil);
    }
    ...
    if (selector == deallocSelector) {
        ...
        NSString *methodEncoding = VD_method_encoding(instanceMethod);
        VDInterceptWithOptimizedObserver optimizedIntercept = optimizedObserversByMethodEncoding[methodEncoding];

        if (!VD_method_has_supported_return_type(instanceMethod)) {
            VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                               code:VDObjCRuntimeErrorObservingMessagesWithUnsupportedReturnType
                                           userInfo:nil], nil);
        }
        ...
        // default fallback to observing by forwarding messages
            ...
            if ([self forwardingSelector:selector forClass:swizzlingImplementorClass]) {
                return VD_default_target_implementation();
            }
    ...
    VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                       code:VDObjCRuntimeErrorUnknown
                                   userInfo:nil], nil);
}
...
-(Class __nullable)prepareTargetClassForObserving:(id __nonnull)target error:(NSErrorParam)error {
    ...
    if (isThisTollFreeFoundationClass) {
        VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                           code:VDObjCRuntimeErrorCantInterceptCoreFoundationTollFreeBridgedObjects
                                       userInfo:nil], nil);
    }
     ...
     * `sentMessage` sets object class to be `_VD_namespace___KVO__OriginalClass` (subclass of `__KVO__OriginalClass`)
    ...
    if ([target class] != object_getClass(target)) {
        ...
        VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                           code:VDObjCRuntimeErrorObjectMessagesAlreadyBeingIntercepted
                                       userInfo:@{
                                                  VDObjCRuntimeErrorIsKVOKey : @(isKVO)
                                                  }], nil);
    }
    ...
    if (previousClass != wannaBeClass) {
        ...
        VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                           code:VDObjCRuntimeErrorThreadingCollisionWithOtherInterceptionMechanism
                                       userInfo:nil], nil);
    }
...
-(BOOL)observeByForwardingMessages:(Class __nonnull)swizzlingImplementorClass
...
#endif
    SEL rxSelector = VD_selector(selector);
    ...
    if (!class_addMethod(swizzlingImplementorClass, rxSelector, implementation, methodEncoding)) {
        VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                           code:VDObjCRuntimeErrorSavingOriginalForwardingMethodFailed
                                       userInfo:nil], NO);
    }
    ...
    if (!class_addMethod(swizzlingImplementorClass, selector, _objc_msgForward, methodEncoding)) {
        if (implementation != method_setImplementation(instanceMethod, _objc_msgForward)) {
            ...
            VD_THROW_ERROR([NSError errorWithDomain:VDObjCRuntimeErrorDomain
                                               code:VDObjCRuntimeErrorReplacingMethodWithForwardingImplementation
                                           userInfo:nil], NO);
        }
    }
...
-(Class __nullable)ensureHasDynamicFakeSubclass:(Class __nonnull)class error:(NSErrorParam)error {
    ...
    NSString *dynamicFakeSubclassName = [VD_PREFIX stringByAppendingString:NSStringFromClass(class)];
...
NSInteger VD_number_of_dynamic_subclasses() {
...
NSInteger VD_number_of_forwarding_enabled_classes() {
    __block NSInteger count = 0;
    [[VDObjCRuntime instance] performLocked:^(VDObjCRuntime * __nonnull self) {
...
NSInteger VD_number_of_intercepting_classes() {
    __block NSInteger count = 0;
    [[VDObjCRuntime instance] performLocked:^(VDObjCRuntime * __nonnull self) {
...
NSInteger VD_number_of_forwarded_methods() {
    return numberOfForwardedMethods;
}

NSInteger VD_number_of_swizzled_methods() {
