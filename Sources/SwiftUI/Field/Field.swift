...
open class UIField<Input: View>: UITextField, UITextFieldDelegate {
	...
	fileprivate var selectedRange: Range<Int> {
		get { range(from: selectedTextRange) }
		set { selectedTextRange = textRange(from: newValue) }
	}
	
	fileprivate func setTextAndCursor(_ string: String, selected: UITextRange? = nil) {
	...
	fileprivate func range(from range: UITextRange?) -> Range<Int> {
	...
	fileprivate func textRange(from range: Range<Int>) -> UITextRange? {
		textRange(
